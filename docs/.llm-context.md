...
- `npm run db:migrate` - Migrate JSON data to database (backend/src/db/migrate.ts)

## Database Migration from JSON

The project includes a migration script that imports data from JSON files into the Prisma database:
...
**Important**: On fresh deployments, ensure database schema exists first:
```bash
npx prisma generate  # Generate Prisma Client
npx prisma db push   # Create database tables
npm run db:migrate   # Import JSON data
```

The migration script:
- Automatically backs up existing SQLite database
- Handles missing tables gracefully (P2021 error)
- Converts numeric IDs to UUIDs
- Imports: artists, paintings, exhibitions, news, shop items, orders, pickup points
- Creates proper relations (artist→paintings, exhibition→paintings, order→items)
