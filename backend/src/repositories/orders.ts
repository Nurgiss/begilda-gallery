...
function toApiFormat(row: OrderWithItems | null): Order | null {
  ...
  return {
    ...
    deliveryType: (row.deliveryType as 'pickup' | 'delivery' | null) ?? undefined,
    ...
    items: row.items.map((item) => ({
      itemId: item.itemId,
      itemType: item.itemType as 'painting' | 'shop',
      title: item.title ?? undefined,
      price: item.price ?? undefined,
      quantity: item.quantity,
    })),
    totalAmount: row.totalAmount ?? undefined,
    status: row.status as 'pending' | 'processing' | 'completed' | 'cancelled',
