      ...
      <div className="container">
        <div className="header-content">
          ...
          <nav className="nav">
            ...
            <Link to="/artists" className={`nav-link-dark ${isActive('/artists') ? 'active' : ''}`}>Artists</Link>
            <Link to="/news" className={`nav-link-dark ${isActive('/news') ? 'active' : ''}`}>News</Link>
            <Link to="/catalog" className={`nav-link-dark ${isActive('/catalog') ? 'active' : ''}`}>Catalog</Link>
            <Link to="/shop" className={`nav-link-dark ${isActive('/shop') ? 'active' : ''}`}>Shop</Link>
            <Link to="/cart" className="nav-link-dark cart-link" style={{ position: 'relative' }}>
              <ShoppingCart size={20} />
              {totalItems > 0 && <span className="cart-badge">{totalItems}</span>}
            </Link>
            <div ref={menuRef} className="currency-group">
              <button className="currency-btn" onClick={() => setShowCurrencyMenu(!showCurrencyMenu)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'inherit' }}>
                <DollarSign size={18} /> <span className="currency-label">{currency}</span>
              </button>
              <button className="currency-chevron" onClick={() => setShowCurrencyMenu(!showCurrencyMenu)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'inherit' }}>
                <ChevronDown size={16} />
              </button>
              {showCurrencyMenu && (
